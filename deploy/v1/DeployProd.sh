...
deploy()
{
    ...
    if [ "$?" == "0" ]; then
        ...
        if [ "$?" == "0" ]; then
            ...
            sendTelegram "😕😕😕😕$nl👀The artifacts cannot be delivered.👀$nl System will continue and try to update the binareis."
        fi
...
waitNextChange()
{
    while [ true ]; do
        ...
        cd $workdir
        local c1=$(git log -n 1 $monitoringBranchName --pretty=format:"%H")
        git pull
        local c2=$(git log -n 1 $monitoringBranchName --pretty=format:"%H")
