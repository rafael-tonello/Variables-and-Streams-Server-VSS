...
#       - withCheckOfFileExistenceAfterExtensionChange: check if the file exists after changing the 
# extension
...
this_listAllCppFiles(){
    ...
    if [ $this_excludeTestsFolder == true ]; then
        this_CppFiles=($(find . -type f -name "*.cpp" -o -name "*.h" | grep -v ".git" | grep -v "/tests/"))
    else
        this_CppFiles=($(find . -type f -name "*.cpp" -o -name "*.h" | grep -v ".git"))
    fi
}
...
withCheckOfFileExistenceAfterExtensionChange(){
