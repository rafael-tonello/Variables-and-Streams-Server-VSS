...
std::string Utils::getAName(void* p, NameType typeOfName, int AlgoGenMaxSyllables) {
    if (!p) return "null";

    std::lock_guard<std::mutex> lock(getANameMutex);

    auto it = getANameDB.find(p);
    if (it != getANameDB.end())
        return it->second;

    std::string name;
    if (typeOfName == ALGORITHM_GENERATED)
        name = Utils::generateAlgorithmicName(AlgoGenMaxSyllables);
    else
        name = Utils::generateHumanLikeName();

    getANameDB[p] = name;
    return name;
}

// Gera nomes tipo “pronunciáveis”: consoante + vogal
std::string Utils::generateAlgorithmicName(int syllables) {
    ...
    std::uniform_int_distribution<size_t> cdist(0, consonants.size() - 1);
    std::uniform_int_distribution<size_t> vdist(0, vowels.size() - 1);

    std::ostringstream oss;
    for (int i = 0; i < syllables; ++i) {
        oss << consonants[cdist(rng)] << vowels[vdist(rng)];
    }
...
// Gera nomes “humanos”
std::string Utils::generateHumanLikeName() {
    static const std::vector<std::string> firstNames = {
        "Aria", "Liam", "Noah", "Mila", "Leo", "Ella", "Ethan", "Nora"
    };
    static const std::vector<std::string> lastNames = {
        "Silva", "Miller", "Johnson", "Pereira", "Smith", "Costa", "Brown"
    };

    static thread_local std::mt19937 rng{std::random_device{}()};
    std::uniform_int_distribution<size_t> fdist(0, firstNames.size() - 1);
    std::uniform_int_distribution<size_t> ldist(0, lastNames.size() - 1);

    return firstNames[fdist(rng)] + " " + lastNames[ldist(rng)];
}
